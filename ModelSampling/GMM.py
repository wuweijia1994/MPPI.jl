...
def logsum(vec, axis=0, keepdims=True):
    ...
    # LOGGER.debug("maxva: {}".format(maxv))
    # LOGGER.debug(maxv)
    maxv[maxv == -float('inf')] = 0
    # LOGGER.debug("maxv == -float('inf')", maxv == -float('inf'))
    # LOGGER.debug("maxv[maxv == -float('inf')]", maxv[maxv == -float('inf')])
    # LOGGER.debug("np.exp(vec-maxv)", np.exp(vec-maxv))
    # LOGGER.debug("np.sum(np.exp(vec-maxv), axis=axis, keepdims=keepdims)", np.sum(np.exp(vec-maxv), axis=axis, keepdims=keepdims))

    return np.log(np.sum(np.exp(vec-maxv), axis=axis, keepdims=keepdims)) + maxv

def test_logsum():
    print(logsum(np.asarray([[0, 1], [2, 3], [4, 6]]), 1))
...
class GMM(object):
    ...
    def update(self, data, K, max_iterations=100):
        ...
        for itr in range(max_iterations):
            ...
            LOGGER.debug("logsum(logobs, axis=1):\n{}".format(logsum(logobs, axis=1)))
            LOGGER.debug("ll:\n{}".format(ll))
...
def test_update():
    ...
    N = 10
    Do = 3
    data = np.random.randint(0, 8, [N, Do])
    gmm.update(data, K)
    # print()
